...
def get_url(soup):
    for item in soup.find_all("a", {'class' : re.compile(r'tapItem fs\-unmask result job_[0-9abcde]+[\w\s]*')}):
        a=item.get('href')
        print("https://fr.indeed.com"+ a)

        
    

url = "https://fr.indeed.com/jobs?q&l=Paris%20(75)&start=10&vjk=a13d9df204cf6f46"
soup = html_code(url)
print(get_url(soup))
